...
Given: 시나리오에 진행에 필요한 모든 준비 과정 (객체, 값, 상태 등등) </br>
When: 시나리오 행동 진행 </br>
Then: 시나리오 진행에 대한 결과 명시, 검증 </br>

## 6. 통합 테스트 

What: 여러 모듈이 협력하는 기능을 통합적으로 검증하는 테스트 </br>
Why: 일반적으로 작은 범위의 unit test만으로 기능 전체의 신뢰성을 보장할 순 없다. </br>
How: 풍부한 단위 테스트와 시나리오 기반의 큰 기능 단위를 검증하는 통합 테스트 구성으로 테스트 코드를 작성한다. </br>
...
#### 학습 테스트는 저장소에 남겨두는가? 
학습테스트는 학습후 삭제하면 된다. </br>
하지만 학습한 테스트한 내용을 팀에 공유하려면 남겨도 좋다. </br>
나는 지우면 아까우니 학습 테스트하고 Gist에 저장해두면 좋을거같다. </br>

## 14. Spring REST Docs

What: 테스트 코드를 통한 API 문서 자동화 도구이다. </br>
Why: API 명세를 문서로 만들고 외부에 제공하여 API 스펙을 공유하며 히스토리를 남길 수 있다. </br>
How: 기본적으로 AsciiDoc을 사용하여 문서를 작성한다. </br>

## 15. REST Docs vs Swagger

장단점에 맞게 잘 선택해서 사용하자.

|   | REST Docs                                                 | Swagger                                                       |
|---|-----------------------------------------------------------|---------------------------------------------------------------|
| 장점 | 1. 테스트를 통과해야 문서가 만들어져 신뢰도가 높다. </br> 2. 프로덕션 코드에 침투하지 않는다. | 1. 문서에 바로 API 호출을 수행해 볼 수 있다. </br> 2. 적용이 쉽다.  </br> 3. 이쁘다. |
| 단점 | 1. 코드 양이 많다. </br> 2. 설정이 어렵다. </br> 3. 이쁘지 않다.           | 1. 테스트와 무관하기 때문에 신뢰도가 떨어질 수 있다. </br> 2. 프로덕션 코드에 침투적이다.      |

## 16. 테스트를 방해하는 요소

1. 귀찮음: 테스트 작성하는것이 귀찮을 수 있다.
   1. 실용적 해결법: SonarQube와 같은 정적 코드 분석툴을 이용해서 테스트 작성을 안하면 배포가 안되도록 막는다.
   1. 마인드 해결법: "테스트를 왜 작성해야하는지"를 생각해보고 즐겁게 작성해보자.
1. 시간: 실무에서 급박한 일정가운데 테스트 코드를 작성하기 어려울 수 있다. 
   1. 실용적 해결법: 테스트 코드를 많이 연습해서 손에 익혀두자.
   1. 마인드 해결법: 1~2시간 더 투자하면 나중에 하루 이상을 아낄수 있다. 재대로 만드는것이 제일 빠른길이다.
...
### Live Template 세팅
Edit창에 tdd 입력으로 템플릿 뜰 수 있도록 만들기
